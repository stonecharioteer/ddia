...
@click.option('--users', default=5000, help='Number of users to generate')
@click.help_option('-h', '--help')
...
def postgres_suggest_follows(user_id):
    ...
    try:
        click.echo(f"Finding follow suggestions for user {user_id} (friends-of-friends)...")
...
@click.option('--min-followers', default=12, help='Minimum followers threshold for influential users')
@click.help_option('-h', '--help')
def postgres_list_influential_followers(min_followers):
    ...
    try:
        click.echo(f"Finding users with influential followers (min {min_followers} followers each)...")
...
def mongo_populate_data(resumes):
    ...
    try:
        ...
        with tqdm(total=resumes, 
                  desc="Inserting resumes", 
                  unit="resume",
                  bar_format="{l_bar}{bar}| {n:_}/{total:_} [{elapsed}<{remaining}]") as pbar:
